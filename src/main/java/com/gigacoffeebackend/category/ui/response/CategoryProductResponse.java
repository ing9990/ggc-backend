...
public class CategoryProductResponse {
    ...
    public static CategoryProductResponse from(Category category) {
        ...
        return CategoryProductResponse.builder().name(category.getName()).products(toProductResponse(category)).build();
    }

    private static Set<ProductResponse> toProductResponse(Category category) {
        return category.getProducts().stream().map(ProductResponse::from).collect(Collectors.toSet());
    }
}
